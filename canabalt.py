...
class Platform(pg.sprite.Sprite):
    ...
    def __init__(self, x, y, color, image : pg.surface.Surface, rest_area = 0, fog = None):
        ...
        if rest_area:
            ...
            x = (100 - msg.get_width()) / 2  # TODO: fix this
            y = (HEIGHT-y - msg.get_height()) / 2
            self.image.blit(msg, (x, y))
    
    @staticmethod
    def rest_area(x, y, image, level):
        return Platform(x, y, GREEN, image, rest_area = level)
...
class Game():
    ...
    def restart(self):
        ...
        self.level += 1
        self.platforms = [Platform.rest_area(player_x, 450, self.platform_image, self.level)]
        ...
        background = [Platform(100 + i*500, HEIGHT // 2, random_color(), self.platform_image, fog = BLACK) for i in range(10)]
    ...
    def construct_platforms(self, count=5):
        ...
        for _ in range(count):
            ...
            self.platforms.append(Platform(x, y, random_color(), self.platform_image))
        ...
        self.platforms.append(Platform.rest_area(*self.platforms[-1].topright, self.platform_image, self.level + 1))
...
class Platform1(pg.sprite.Sprite):
    ...
    def rest_area(x, y, level):
        return Platform1(x, y, 300, GREEN, rest_area = level)
...
class Game1():
    ...
    def restart(self):
        ...
        self.level += 1
        self.platforms = [Platform1.rest_area(*pos, self.level)]
        self.construct_platforms()
        self.level += 1
        self.construct_platforms()
        self.level += 1
    ...
    def construct_platforms(self, count=5):
        ...
        for _ in range(count):
            x, y = self.get_next_position(*self.platforms[-1].topright)
            self.platforms.append(Platform1(x, y, self.platform_width, random_color()))
            # self.platforms.append(Platform(x, y()))
            ...
            if random.random() < self.obstacle_density:
                ...
                # self.obstacle_sprites.add(Obstacle(x + self.platform_width // 2, y, self.obstacle_image))
                # self.obstacle_sprites.add(Obstacle(x + self.platform_width // 2, y, self.obstacle_size))
        self.platforms.append(Platform1.rest_area(*self.platforms[-1].topright, self.level + 1))
